...
for (const r of readings) {
  if (r.skipped) {
    console.log(r.t.padEnd(10), 'skipped');
    continue;
  }
  if (r.ratio == null) {
    console.log(r.t.padEnd(10), 'no ratio  fg=', r.color, ' bg=', r.bg);
    continue;
  }
...
await page.screenshot({
  path: resolve(out, 'typefilter-light.png'),
  clip: { x: 0, y: 200, width: 1280, height: 220 },
});
await ctx.close();
await browser.close();
