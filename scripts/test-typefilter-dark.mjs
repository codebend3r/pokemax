...
await page.evaluate(() => {
  ...
  try { localStorage.setItem('pokemax-theme', 'dark'); } catch {}
});
...
function parseColor(s) {
  ...
  if (m) return m[1].split(/\s+/).map(x => parseFloat(x) * 255).slice(0,3);
  return null;
}
function lum(rgb) {
  const a = rgb.map(v => { v=v/255; return v<=0.03928 ? v/12.92 : Math.pow((v+0.055)/1.055,2.4); });
  return 0.2126*a[0]+0.7152*a[1]+0.0722*a[2];
}
function contrast(c1,c2) { const l1=lum(c1),l2=lum(c2); return l1>l2 ? (l1+0.05)/(l2+0.05) : (l2+0.05)/(l1+0.05); }

const types = ['normal','fire','water','bug','rock','steel','ice','electric','ghost','dark'];
...
for (const t of types) {
  ...
  const fg = parseColor(c.color), bg = parseColor(c.bg);
