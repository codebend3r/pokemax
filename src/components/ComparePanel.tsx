...
function TypeChip({ name }: { name: string }) {
  ...
  return (
    <span className="crt-type" style={{ color, borderColor: color, textShadow: `0 0 4px ${color}66` }}>
      {name}
    </span>
  );
}
...
export default function ComparePanel({ base, species, onClose }: Props) {
  ...
  const suggestions = useMemo(() => {
    ...
    if (!q) return [];
    return species
      .filter((s) => s.name !== base.name && s.name.includes(q))
      .slice(0, 10);
  ...
  useEffect(() => {
    ...
    fetchPokemon(target.id)
      .then((p) => { if (active) setTargetData(p); })
    ...
    return (
      <div className="crt-compare">
        <div className="crt-compare-header">
          ...
          <button type="button" className="crt-compare-close" onClick={onClose}>[ × ]</button>
        </div>
        <input
  ...
  if (error) {
    return (
      <div className="crt-compare">
        <div className="crt-compare-header">
          <span className="crt-compare-label">▶ ERR LOADING {pretty(target.name).toUpperCase()}</span>
          <button type="button" className="crt-compare-close" onClick={onClose}>[ × ]</button>
        </div>
      </div>
    );
  ...
  return (
    <div className="crt-compare">
      <div className="crt-compare-header">
        ...
        <button type="button" className="crt-compare-change" onClick={() => { setTarget(null); setTargetData(null); }}>[ change ]</button>
        <button type="button" className="crt-compare-close" onClick={onClose}>[ × ]</button>
      </div>

      <div className="crt-compare-row crt-compare-names">
        <div className="crt-compare-col">
          <img src={`${SHOWDOWN_BASE}/${base.id}.gif`} alt={base.name} className="crt-compare-sprite" />
          <div className="crt-compare-name">{pretty(base.name).toUpperCase()}</div>
          <div className="crt-compare-types">
            {base.types.map((t) => <TypeChip key={t.type.name} name={t.type.name} />)}
          </div>
        </div>
        <div className="crt-compare-vs">VS</div>
        <div className="crt-compare-col">
          <img src={`${SHOWDOWN_BASE}/${targetData.id}.gif`} alt={targetData.name} className="crt-compare-sprite" />
          <div className="crt-compare-name">{pretty(targetData.name).toUpperCase()}</div>
          <div className="crt-compare-types">
            {targetData.types.map((t) => <TypeChip key={t.type.name} name={t.type.name} />)}
          </div>
        </div>
      </div>
        ...
        {STAT_ORDER.map((key) => {
          ...
          return (
            <div key={key} className="crt-compare-stat-row">
              <span className={'crt-compare-stat-num' + (diff > 0 ? ' win' : diff < 0 ? ' lose' : '')}>{a}</span>
              <span className="crt-compare-stat-label">{STAT_LABELS[key] ?? key.toUpperCase()}</span>
              <span className={'crt-compare-stat-num' + (diff < 0 ? ' win' : diff > 0 ? ' lose' : '')}>{b}</span>
            </div>
          );
        })}
        <div className="crt-compare-stat-row total">
          <span className={'crt-compare-stat-num' + (baseTotal > targetTotal ? ' win' : baseTotal < targetTotal ? ' lose' : '')}>{baseTotal}</span>
          <span className="crt-compare-stat-label">TOTAL</span>
          <span className={'crt-compare-stat-num' + (targetTotal > baseTotal ? ' win' : targetTotal < baseTotal ? ' lose' : '')}>{targetTotal}</span>
        </div>
      </div>
      ...
      <div className="crt-compare-vitals-row">
        ...
        <div className="crt-compare-vital-cell">
          ...
          <span className="crt-compare-vital-label">WT</span> {(targetData.weight / 10).toFixed(1)}kg
        </div>
      </div>
    </div>
  );
